...
func TestData(t *testing.T) {
	...
	fmt.Printf("key pair converted to native: %s, type: %s, typeFnc: %s, typeNat: %s\n",
		pair, pair.Type(), pair.TypeFnc(), pair.Type())

	var nest = DecData(d.NewPair(d.StrVal("key"), d.NewPair(d.StrVal("inner key"), d.StrVal("Value"))))
